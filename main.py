...
parser.add_argument('--decays', nargs='+', type = float, default = [0.0, 0.0, 0.0, 0.0], metavar = 'd', help='layer wise decay')
...
parser.add_argument('--gdu',type = bool, default = True, metavar = 'g', help='checking the gdu while training')
...
print('#################################')
print('\nargs\tmbs\tpool\tT1\tT2\tepochs\tactivation\tbetas')
print('\t',args.mbs,'\t',args.pool,'\t',args.T1,'\t',args.T2,'\t',args.epochs,'\t',args.act, '\t', args.betas)
print('\n')
...
if args.task=='MNIST':
    ...
    mnist_dset_train = torchvision.datasets.MNIST('./mnist_pytorch', train=True, transform=transform, target_transform=None, download=False)
    train_loader = torch.utils.data.DataLoader(mnist_dset_train, batch_size=mbs, shuffle=True, num_workers=1)

    mnist_dset_test = torchvision.datasets.MNIST('./mnist_pytorch', train=False, transform=transform, target_transform=None, download=False)
...
if args.todo=='train':
    if len(args.lrs)==1:
        optimizer = torch.optim.SGD(model.parameters(), lr = args.lrs[0])
    elif len(args.lrs)==len(model.synapses):
        optim_params = []
    ...
    print(optimizer)
    train(model, optimizer, train_loader, test_loader, args.T1, args.T2, betas, device, epochs=args.epochs, criterion=criterion, check_thm=args.gdu, path=path)
